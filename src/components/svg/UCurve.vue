<template>
    <div class="relative w-full" :style="{height: height + 'px'}">
        <svg :width="width" :height="height" viewBox="0 0 1200 300" xmlns="http://www.w3.org/2000/svg">
        ...
        >
            <div class="rounded-full db_overlay-image" :style="isDesktop ? {width: '54px', height: '54px'} : {width: '66px', height: '66px'}">
                <img :src="doctor.img" :style="isDesktop ? {width: '52px', height: '52px'} : {width: '64px', height: '64px'}" 
                    class="rounded-full object-top object-cover bg-white"/>
            </div>
    ...
    computed: {
        paths() {
            return [0, 1, 2].map(index => 
                `M0,${index * 90} Q${300 + index * 50},${300 - index * 10} ${this.width},${index * 80}`
            );
        },
        doctors() {
            ...
            const fixedPositions = [0.18, 0.32, 0.49, 0.62, 0.77, 0.90];
