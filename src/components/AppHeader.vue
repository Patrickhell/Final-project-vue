        ...
        <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
            ...
            <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1 bd_nav-items-list" id="navbar-sticky">
                <ul class="flex flex-col gap-2 p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                    ...
                    <li>
                        <a href="" 
                            @click.prevent="handleServiceLinkClick" 
                            class="block py-2 pl-3 pr-4 rounded md:p-0 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700 bd_nav-item">
                              Servizi
                        </a>
                    </li>
...
export default {
    ...
    methods: {
        ...
        handleDoctorLinkClick() {
            if (this.$route.path === "/") {
                const section = document.getElementById('listDoctors');
                if (section) section.scrollIntoView({ behavior: "smooth" });
            } else {
                this.$router.push("/list-doctors");
            }
        },
        handleServiceLinkClick() {
            if (this.$route.path === "/") {
                const section = document.getElementById('servicePlans');
                if (section) section.scrollIntoView({ behavior: "smooth" });
            } else {
                this.$router.push({ path: '/', query: { scrollToService: true } }).then(() => {
                    const section = document.getElementById('servicePlans');
                    if (section) section.scrollIntoView({ behavior: "smooth" });
                });
            }
        },
        // Check if the user scrolling vertical action is greater then 0
        handleScroll() {
            if (window.scrollY > 0) {
...
nav {
    ...
    &.bd_nav-shadow {
        box-shadow: 0px 4px 16px -2px $nav-shadow-color;
        -webkit-box-shadow: 0px 4px 16px -2px $nav-shadow-color;
        -moz-box-shadow: 0px 4px 16px -2px $nav-shadow-color;
        -moz-box-shadow: 0px 4px 16px -2px $nav-shadow-color;
    }
}
